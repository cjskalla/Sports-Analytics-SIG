...
hustle_data <- select(data, -FG, -FGA, -FGPCT, -threePM, -threePA, 
...
```{r, warning=FALSE, message=FALSE}
EFGPCT_data <- mutate(data, "EFGPCT" = round(((twoPM + (1.5*threePM))/(FGA)), digits = 3)*100)
...
### Using different Machine Learning Techniques to classify playoff teams
#### Building a Random Forest to classify playoff teams
```{r, warning=FALSE, message=FALSE}
library(tree)

num.data <- select(data, -Rank, -Team, -G, -MP, -Year)

playoff.tree <- tree(as.factor(Playoff) ~ ., num.data)
summary(playoff.tree)
plot(playoff.tree)
text(playoff.tree, pretty = 0)
```
<hr>
...
svm.data <- select(num.data, Playoff, twoPPCT, TOV) %>% filter() 
